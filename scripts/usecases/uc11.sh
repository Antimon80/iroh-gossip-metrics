...
# UC11: Churn and Recovery, degraded network
#
# Direct discovery, degraded network (delay and loss). Bootstrap set scales with peer count (1 per 10 peers, ceil).
...
# Network scenario on the bridge (50 ms delay, 30 % package loss)
...
BASELOG="${LOGDIR:-logs/uc11}/p${PEERS}/c${CHURN_PCT}"

# Create parameter-tagged run directory
TS=$(date +"%Y%m%d-%H%M%S")
TAG="uc11_p${PEERS}_c${CHURN_PCT}_m${NUM}_r${RATE}_s${SIZE}_${DISCOVERY}"
RUN_ID="run-${TS}_${TAG}"
...
echo "== UC11 Churn & Recovery with $PEERS peers (Direct discovery) =="
