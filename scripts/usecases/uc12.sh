...
# UC12: Churn and Recovery (degraded network)
#
# Identical to UC11, but Discovery over Relay.
...
BASELOG="${LOGDIR:-logs/uc12}/p${PEERS}/c${CHURN_PCT}"

# Create parameter-tagged run directory
TS=$(date +"%Y%m%d-%H%M%S")
TAG="uc12_p${PEERS}_c${CHURN_PCT}_m${NUM}_r${RATE}_s${SIZE}_${DISCOVERY}"
RUN_ID="run-${TS}_${TAG}"
...
echo "== UC12 Relay Churn & Recovery with $PEERS peers (Relay discovery) =="
